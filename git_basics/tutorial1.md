...
If git is not recognized as an internal or external command we need to set envrionment varibale called Path to contain Git installation paths.
Go to Control Panel -> System and Security -> System and open Advanced system settings. In the Advanced system settings window go to tab Advanced and click on Environment Varibales..
Edit System varibale called Path to contain Git installation paths:
...
Note that if you call this command in your local repository list can containt more and different configuration setup.
...
You can intialize your local git repository in this folder by running the command `git init` and then `git remote add origin git@bitbucket.org:your_username/your_repo_name.git` to add your repository as a remote repository.
...
Note: To navigate through commit hitsory use `arrow keys` and to quit type `:q` if the list is longer.
...
Now let's regiter our repository branch to our helper server `GitCommander`. This server will provide some services that will help us to simulate the work of another developer on the same code taht we are working on.

The registration will help server to clone our project and commit and push the changes everytime we call some of his execute commands/actions.

For registration we will need to send information about our git user name, url to of our repository and branch namme that we are working on.
...
To register call registrer URL with your parameters in place of `<brackets>`

> http://localhost:8080/GitCommander/GitServlet?action=register&user=<your_bitbucket_username>&url=<gitrepourl>&branch=<your_branch_name>

For this tutorial the `<gitrepourl>` will be `https://bitbucket.org/zuehlke/gitworkshop.git`

If you want to check your registries (maybe to retrive forgotten ID) you can call GitCommander's action url:

>http://localhost:8080/GitCommander/GitServlet?action=getMyRegistries&user=<your_bitbucket_username>

This call will retrive the list of your registries togheter with git URLs and branchnames with dedicated IDs.

If you made a typo with some registration or just simply want to unregister you can do that with the command:

>http://localhost:8080/GitCommander/GitServlet?action=unregister&ID=<your_registration_ID>&user=<your_bitbucket_username>
...
Staging the file means that we are marking this file to be commited.
...
`git commit -m "I have discovered Mercury guys"` - This command commits staged files into our local repository. The parameter `-m` is a Commit message that we want to attach to our commit so that outher developers can have easier understanding what have we pushed to the repository.
...
To synchornize the state of local reposiotry with remote repository first we need to pull all the changes from remote repository in order not to override the change of other developers (in this case planet explorers).
...
>Note: git push command has a prequest to be completed successfully and that is that all remote changes are synchronized to the local repository. This means that the command will complete if nobody else pushed some change sto the remote repository. In other case you will get rejected message and a suggestion that the command git pull must be complited successfully before proceeding.
...
`git add .` - In case when you have multiple changes that you what to stage and commit you can stage all files at once by calling this command instead of calling the add commang for every file individually.
...
Let's check what discoveries have our coleague made.

Call GitCommander to discover other planets using url:

>http://localhost:8080/GitCommander/GitServlet?action=discoverFourBigBrothers&ID=<your_registration_ID>
...
OK... It seems that there is some "planet" file called pluto.js that was missplaced in our solar system. Nevermind then we will handle this later.

As we already explained the common usage of command `git log` from now onwards `git log` is optional.
...
As we already demostrated the common usage of command `git status` from now onwards this command is optional.
...
>http://localhost:8080/GitCommander/GitServlet?action=removeFile&ID=<your_registration_ID>&fileName=pluto.js

Now will execute the standard set of commands to push our changes up to the remote repository:
You can eihter `git add .` or `git add pluto.js` to stage our changed file because it is the only change in our working directory so the effect is the same.
...
>2fa4a5a6760d3653509e74 and modified in HEAD. Version HEAD of solar system/js/plu
>to.js left in tree.
...
Execute this block of commands to reintorduce the pluto.js into project again:
`git add .`
`git commit - m "merging pluto.js conflict"`
`git pull`
`git push`

>Note: this can be handy if somebody accidently removes some file that is still needed in the project. This is one of the ways to retreive the file state.
...
>http://localhost:8080/GitCommander/GitServlet?action=revokePluto&ID=<your_registration_ID>

This will edit information about pluto in `index.html`.

Now edit `index.html` to show the information about pluto from `pluto.js`. The `index.html` shoudl have body content like this:
...
>=========
>	remote code
>>>>>>>> master hashcode of last commit
...
In orde to understand better the conflict resolving we will fix this conflict by hand with simple text editor.
...
As you can see we have successfully resolved our first merge conflict, even by hand. We still belive that the Pluto is a planet. Keep you houps up Pluto!
...
For this purpose we will use `git checkout` command which will copy the desired file in our working directory. In our case we wan't to checkout the file for the specific commit state.
...
It supports:
* File deletion: `http://localhost:8080/GitCommander/GitServlet?action=removeFile&ID=<your_registration_ID>&fileName=<filename.fileextention>`
* Adding random content as new line in file: `http://localhost:8080/GitCommander/GitServlet?action=addNewLineToFile&ID=<your_registration_ID>&fileName=<filename.fileextention>`
* File line editing: `http://localhost:8080/GitCommander/GitServlet?action=editFile&ID=<your_registration_ID>&fileName=<filename.fileextention>&lineNumber=<file_line_number_to_edit>&content=<content_to_write_in_file>`
* Adding new random named file: `http://localhost:8080/GitCommander/GitServlet?action=addNewFile&ID=<your_registration_ID>`

Try recreating the conflict scenarios and resolve them.

Never hesitate to ask a coulleage for help if needed. :)